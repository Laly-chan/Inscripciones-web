...
const cursos = [
    ...
    query('cantidadHoras')
        .optional()
        .notEmpty().withMessage('apellido no puede estar vacío')
        .isInt({ min: 0 }).withMessage('cantidad de horas debe ser un entero no negativo')
        .toInt(),
    query('inscriptosMax')
        .optional()
        .isInt({ min: 0 }).withMessage('inscriptos max debe ser un entero no negativo')
        .toInt(),
    (req, res, next) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
